...
| 名称 | 目的 | 当前来源 |
| --- | --- | --- |
| `oracle-source.asc` | 原 CTB payload oracle、边界重采样与 child mask | 已提交；checksum 与生成命令见 `tests/fixtures/MANIFEST.md` |
| `epsg4326-small-int16.tif` | 65×65 边界、负值与 `i16` 高程 | 待在 P1b 中程序化生成 |
...
Fixture 引入前必须记录生成脚本、许可证、checksum 和预期元数据；权威记录位于 `tests/fixtures/MANIFEST.md`。不能将大体积或来源不明的 DEM 直接提交。
